...
async def random_wujie_qixue(kungfu: str) -> tuple[list[str], list[str]]:
    ...
    for version_data in wujie_index:
        if version_data["name"].find("体服") == -1:
            ...
            qixue_data: dict[str, dict[str, dict[str, dict[str, Any]]]] = (
                await Request(f"https://data.jx3box.com/talent/wujie/{version_key}.json").get()).json()
    ...
    async def get_qixue_data() -> dict:
        ...
        for each_ver in data:
            if each_ver["name"].find("体服") == -1:
                qixue_data = (
                    await Request("https://data.jx3box.com/talent/std/" + each_ver["version"] + ".json").get()).json()
        ...
        for x in self.qixue_data[self.kungfu]:
            for y in self.qixue_data[self.kungfu][x]:
                if self.qixue_data[self.kungfu][x][y]["name"] == self.name:
                    return x, y, "https://icon.jx3box.com/icon/" + str(
                        self.qixue_data[self.kungfu][x][y]["icon"]) + ".png"
...
class EquipDataProcesser:
    ...
    special_weapons = ["雪凤冰王笛", "血影天宇舞姬", "炎枪重黎", "腾空", "画影", "金刚", "岚尘金蛇", "苌弘化碧",
                       "蝎心忘情", "抱朴狩天", "八相连珠", "圆月双角", "九龙升景", "斩马刑天", "风雷瑶琴剑", "五相斩",
                       "雪海散华", "麒王逐魂"]
    ...
    @property
    def score(self) -> int:
        return self.data["panelList"]["score"]

    @staticmethod
    def _parse_attributes(data: dict | str) -> str:
        msg = ""
        filter_word = ["全", "阴性", "阳性", "阴阳", "毒性", "值", "成效", "内功", "外功", "等级", "混元性",
                       "水下呼吸时间", "抗摔系数", "马术气力上限", "气力上限"]
        if isinstance(data, dict):
            for i in data["modifyType"]:
                content = i["desc"].split("等级提高")
                if len(content) == 1:
                    content = content[0].split("提高")
                if len(content) == 1:
    ...
    def _format_equip(self, equip_data: dict, location: str) -> Equip:
        ...
        if belong not in ["pve", "pvx", "pvp"]:
            belong = "pve"
        enchant = []
        if "permanentEnchant" in equip_data:
            enchant.append(
                Enchant(
                    icon=build_path(ASSETS, ["image", "jx3", "attributes", "permanent_enchant.png"]),
                    name=equip_data["permanentEnchant"][0]["name"],
                    type="pe"
                )
            )
        if "commonEnchant" in equip_data:
            enchant.append(
                Enchant(
                    icon=build_path(ASSETS, ["image", "jx3", "attributes", "common_enchant.png"]),
                    name=equip_data["commonEnchant"]["name"],
                    type="ce"
                )
            )
        if "colorStone" in equip_data:
            enchant.append(
                Enchant(
                    icon=equip_data["colorStone"]["icon"],
                    name=equip_data["colorStone"]["name"],
                    type="cs"
                )
            )
        ...
        fivestone = [
            FiveStone(
                level=int(fs["level"]),
                attr=self._parse_attributes(str(fs["desc"]))  # type: ignore
            )
            for fs
        ...
        #         effect: str = [a["Attrib"] for a in equip_data["modifyType"] if a["Desc"] == "atSkillEventHandler"][0][
        #             "Desc"].replace("进入战斗后，", "").split("，")[0]
        #         enchant.append(
        #             Enchant(
        #                 icon=build_path(ASSETS, ["image", "jx3", "attributes", "common_enchant.png"]),
        #                 name=
        #                 effect.replace("招式", "").replace("少量", "").replace("自身", "").replace("基础", "").replace(
        #                     "等级", "").split("。")[0],
        #                 type="ce"
        #             )
        #         )
        ...
        peerless = (equip_data["maxStrengthLevel"] == "3") \
                   or (equip_data["maxStrengthLevel"] == "4") \
    ...
    def equips(self) -> list[Equip]:
        ...
        try:
            data = self.data["equipList"]
        except TypeError:
            raise ValueError("玩家似乎在提交角色给音卡后删除了账号！")

        for equip in data:
            subkind = equip["subKind"]
            kind = equip["kind"]
            if subkind == "戒指":
    ...
    async def qixue(self) -> list[Talent]:
        qixue_list = self.data["qixueList"]
        unknown_img = build_path(ASSETS, ["image", "jx3", "attributes", "unknown.png"])
        name = ["未知", "未知", "未知", "未知", "未知", "未知", "未知", "未知", "未知", "未知", "未知", "未知"]
        icon = [unknown_img, unknown_img, unknown_img, unknown_img, unknown_img, unknown_img, unknown_img, unknown_img,
                unknown_img, unknown_img, unknown_img, unknown_img]
    ...
    def panel_types(self) -> list[str]:
        if self.kungfu.base in ["根骨", "元气", "力道", "身法"]:
            return ["面板攻击", "基础攻击", "会心", "会心效果", "加速", self.kungfu.base, "破防", "无双", "破招",
                    "最大气血值", "御劲", "化劲"]
        elif self.kungfu.base == "治疗":
            return ["面板治疗量", "基础治疗量", "会心", "会心效果", "加速", "根骨", "外功防御", "内功防御", "破招",
                    "最大气血值", "御劲", "化劲"]
        elif self.kungfu.base == "防御":
            return ["外功防御", "内功防御", "最大气血值", "破招", "御劲", "闪避", "招架", "拆招", "体质", "加速率",
                    "无双", "加速"]
        else:
            return ["未知属性"] * 12
    ...
    def _panel_type(self, panel_attr_name: str) -> SingleAttr:
        ...
        if panel_attr_name == "加速率":
            speed_percent = True
        for attr in self.data["panelList"]["panel"]:
    ...
    def panel_values(self) -> list[str]:
        ...
        result = []
        if self.data["panelList"].get("panel") is None:
            ...
            if name.endswith("·悟"):
                name = name[:-2]
            result = mobile_attribute_calculator(self.data["equipList"], name, attr_types)
            return result
        if self.kungfu.base is not None and self.data["panelList"]["panel"] is not None:
...
async def get_attr_v2_remake(data):
    equip_data_object = EquipDataProcesser(data, data["roleName"])
    try:
        equip_data_object.equips
    except TypeError:
    ...
    image = await get_attr_v2_remake_img(
        ...
        data["bodyName"],
        data["roleId"],
        kungfu=equip_data_object.kungfu,
        school=School(equip_data_object.kungfu.school),
        equips=equip_data_object.equips,
        talents=(await equip_data_object.qixue()),
        panel=equip_data_object.panel,
        score=equip_data_object.score
    )
    # return image
...
async def get_attr_v2_remake_img(
    ...
    max_strength_unapproching = Image.open(
        build_path(ASSETS, ["image", "jx3", "attributes", "not_max_strength.png"])).resize((38, 38))
    common_enchant_icon = Image.open(build_path(ASSETS, ["image", "jx3", "attributes", "common_enchant.png"])).resize(
        (20, 20))  # 大附魔
    permanent_enchant_icon = Image.open(
        build_path(ASSETS, ["image", "jx3", "attributes", "permanent_enchant.png"])).resize((20, 20))  # 小附魔
